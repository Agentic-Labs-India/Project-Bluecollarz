...
7. Cookie banner is **18+ Agree / Decline**. Decline blocks Log in / Get Started and re-opens the same banner if they try again. Analytics stay **off** until Settings → Allow analytics cookies. Browser analytics choice syncs to `users.cookiesEnabled` on login; rail must not overwrite a prior Allow.
