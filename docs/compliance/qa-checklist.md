...
7. Cookie banner is Grok-style **Agree and continue** / **Reject All** /
   **Cookies Settings**, with 18+ in the copy. Reject All blocks Log in / Get
   Started and re-opens the same banner if they try again. Agree and continue
   does not turn analytics on. Cookies Settings can allow analytics. Browser
   analytics choice syncs to `users.cookiesEnabled` on login; rail must not
   overwrite a prior Allow.
8. Settings → Access / portability downloads JSON export (with authorised file
   paths); `/admin/compliance` Rights queue works (admin notes not returned to
   workers). Correction points to `/candidate/profile`. Erasure is **not**
   auto-delete: log the request, then Delete account (`#delete-account`) after
   identity checks. Grievance SLA: 90 days.
...
10. Account delete removes apps/interviews/blobs/inquiries/consent/rights, hire
    company documents, and that user's legal-safety rows. Blob delete runs
    before Mongo rows. Legal hold blocks delete.
