...
const PRIMARY_COLORS: [string, string] = ["#233eff", "#c5ceff"];
...
function createNoiseTexture() {
  ...
  const grid = 32;
  const cell = grid + 1;
  const grads = new Float32Array(cell * cell);
  let seed = 0x9e3779b9;
  const rand = () => {
    seed = (Math.imul(seed, 16807) % 2147483647) | 0;
    return (seed + 2147483647) / 4294967294;
  };
  for (let i = 0; i < grads.length; i++) grads[i] = rand();

  const data = new Uint8Array(size * size * 4);
  const fade = (t: number) => t * t * (3 - 2 * t);
  const sample = (ix: number, iy: number) => grads[iy * cell + ix] ?? 0;
...
float orderedDither(vec2 coord) {
...
void main() {
    ...
    float interior = 1.0 - smoothstep(0.12, 0.92, radius);
    luminance = clamp(
        luminance + (orderedDither(gl_FragCoord.xy) - 0.5) * 0.28 * interior,
        0.0,
        1.0
    );
